...
bool AiAssistantSegmentor::Proceed(Segmentation* segmentation) {
  ...
  if (HandleClearHistoryShortcut(segmentation, ai_context,
...
void AiAssistantSegmentor::EnsureConfigLoaded() {
  ...
  if (!config) {
    ResetConfigCaches();
    config_loaded_ = false;
    last_schema_id_ = schema->schema_id();
    return;
  }
...
void AiAssistantSegmentor::LoadConfig(Config* config) {
  ...
  if (config->GetBool("translator/keep_input_uncommit", &keep_input_uncommit)) {
    keep_input_uncommit_ = keep_input_uncommit;
  }

  bool commit_question = false;
  if (config->GetBool("ai_assistant/behavior/commit_question",
                      &commit_question)) {
    behavior_.commit_question = commit_question;
  }

  bool auto_commit_reply = false;
  if (config->GetBool("ai_assistant/behavior/auto_commit_reply",
                      &auto_commit_reply)) {
    behavior_.auto_commit_reply = auto_commit_reply;
  }

  bool clipboard_mode = false;
  if (config->GetBool("ai_assistant/behavior/clipboard_mode",
                      &clipboard_mode)) {
    behavior_.clipboard_mode = clipboard_mode;
  }

  std::string prompt_chat;
  if (config->GetString("ai_assistant/behavior/prompt_chat", &prompt_chat)) {
    behavior_.prompt_chat = prompt_chat;
  }

  if (an<ConfigMap> prompts = config->GetMap("ai_assistant/ai_prompts")) {
    for (auto it = prompts->begin(); it != prompts->end(); ++it) {
      const std::string& trigger_name = it->first;
      const std::string base_path = "ai_assistant/ai_prompts/" + trigger_name;
...
bool AiAssistantSegmentor::HandleReplyInput(
  ...
  Segment reply_segment(0, segmentation_input.length());
...
bool AiAssistantSegmentor::HandlePromptSegment(
  ...
  Segment prompt_segment(0, behavior_.prompt_chat.size());
...
bool AiAssistantSegmentor::HandleChatTrigger(
  ...
  Segment ai_segment(0, matched_prefix.size());
